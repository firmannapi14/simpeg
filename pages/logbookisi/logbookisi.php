                ...
                <div class="col-sm-12 col-md-12 col-lg-12">
                    <div class="card card-accent-primary">
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-md-3">
                                <a href="?page=logbookisiadd&nik=<?= $_GET['nik'] ?>&id=<?= $_GET['id'] ?>" class="btn btn-primary"><span class="fa fa-plus-circle"></span> Tambah Data Logbook</a>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <table class="example table table-responsive-xs table-bordered table-striped table-sm">
                                        ...
                                        while($data=mysqli_fetch_array($q)){ ?>
                                            <tr>
                                                ...
                                                <td><?= !empty($data['dokumen']) ? $data['dokumen'] : '-' ?></td>
                                                <td>
                                                    <a href="?page=logbookisiedit&nik=<?= $_GET['nik'] ?>&id=<?= $_GET['id'] ?>&idx=<?= $data['id'] ?>" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> edit</a>
                                                    <a href="?page=logbookisidelete&id=<?= $_GET['nik'] ?>&id=<?= $_GET['id'] ?>&idx=<?= $data['id'] ?>" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> delete</a>
                                                </td>
                                            </tr>
                                        <?php $no++; } ?>
                                        </tbody>
